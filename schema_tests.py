...
@call_once_only
def get_test_data():
    return load_json_file(os.path.dirname(os.path.realpath(__file__)) + "/schema_tests/payload_tests/payload.json")


@call_once_only
def get_schema():
    return load_json_file(os.path.dirname(os.path.realpath(__file__)) + get_from_schema(get_test_data(), "schema"))


def verify_patterns(element, key, obj, idx):
    pattern_path = str("test_patterns/"+key)
    path_to_pattern_in_schema = get_from_schema(get_test_data(), pattern_path+"/pattern_path")
    obj_in_schema = get_from_schema(get_schema(),path_to_pattern_in_schema)
    ...
    test_payloads = get_from_schema(get_test_data(), pattern_path+"/payload_tests")
    print("Checking patterns for '%s'" % key)

    # we need to merge in pattern and enum data from parent context
    # unfortunately, functional library does not support passing in context data
...
def verify_against_schema(element, key, obj):
    ...
    print("%s - checking payload for %s - errors expected: %s, errors received: %s" %
          (error_count == element["expected"], element["description"], element["expected"], str(error_count)))
    return error_count == element["expected"]


def run_checks():

    print("Starting schema tests")
    print("---------------------\n")
    pattern_results = _.every(get_from_schema(get_test_data(), "test_patterns"), verify_patterns)

    print("\nSchema pattern tests passed: %s" % pattern_results)
    ...
    print("Payload tests passed: %s" % payload_results)
